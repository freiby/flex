...
public final class ContainerAccess {
	 ...
	 * The logging object.
	 */
	private static final Log LOG = LogFactory.getLog(ContainerAccess.class);

	/**
	 * The name of the attribute to use when getting and setting the container
	 * object in a context.
	 */
	public static final String CONTAINER_ATTRIBUTE = "com.wxxr.nirvana.CONTAINER";
	...
	public static void setSessionWorkbench(IWorkbench value) {
		NirvanaServletContext
				.getContext()
				.getSession()
				.setAttribute(ContainerAccess.WORKBENCH_SESSION_ATTRIBUTE,
						value);
	}
	 ...
	 */
	public static void setWorkbench(Object context, IWorkbench workbench)
			throws NirvanaException {
		if (workbench == null) {
			if (LOG.isInfoEnabled()) {
				LOG.info("Removing nirvana for context: "
						+ context.getClass().getName());
			}
			removeAttribute(context, WORKBENCH_ATTRIBUTE);
		}
		if (workbench != null && LOG.isInfoEnabled()) {
			LOG.info("Publishing nirvana for context: "
					+ context.getClass().getName());
		}
	 ...
	 */
	public static IWorkbenchContainer getContainer() {
		return (IWorkbenchContainer) NirvanaServletContext.getContext()
				.getSession().getAttribute(ContainerAccess.CONTAINER_ATTRIBUTE);
	}
	 ...
	 * @throws NirvanaException
	 *             If something goes wrong during manipulation of the context.
	 */
	public static void setContainer(IWorkbenchContainer container)
			throws NirvanaException {
		NirvanaServletContext.getContext().getSession()
				.setAttribute(ContainerAccess.CONTAINER_ATTRIBUTE, container);
	}
	 ...
	 * @throws NirvanaException
	 *             If something goes wrong during setting the attribute.
	 */
