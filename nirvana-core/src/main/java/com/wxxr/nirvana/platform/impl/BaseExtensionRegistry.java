...
//	private void fireEvent()
	...
	public boolean addContribution(Object source, IContributor contributor, String name, MessageResourceBundle translationBundle) throws IllegalArgumentException {
		Object obj = context.getObjectFactory().createExtensionObject(this, source);
		...
		if(obj != null){
			if(obj instanceof IExtensionPoint){
				...
				deltas.add(new ExtensionDelta((IExtensionPoint)obj,IExtensionDelta.ADDED));
				resovleUnresolvedExtensions((IExtensionPoint)obj,deltas);
				result = true;
			}else if(obj instanceof IExtension){
				String pid = ((IExtension)obj).getExtensionPointUniqueIdentifier();
				String extid = ((IExtension)obj).getUniqueIdentifier();
				IExtensionPoint p = getExtensionPoint(pid);
				if(p != null){
					context.getObjectFactory().addExtensionTo(p,(IExtension) obj);
					extensions.put(extid, (IExtension) obj);
					deltas = new LinkedList<IExtensionDelta>();
					deltas.add(new ExtensionDelta((IExtension) obj,	IExtensionDelta.ADDED));
					result = true;
				}else{
					LinkedList<IExtension> unresolved = unresolvedExtensions.get(pid);
					if (unresolved == null) {
  ...
  protected void doFireRegistryEvent(IRegistryChangeEvent event) {
	...
	protected void resovleUnresolvedExtensions(IExtensionPoint point,List<IExtensionDelta> deltas){
	  LinkedList<IExtension> unresolveds = unresolvedExtensions.remove(point.getUniqueIdentifier());
	  if((unresolveds == null)||unresolveds.isEmpty()){
	    return;
	  }
	  for (Iterator<IExtension> iterator = unresolveds.iterator(); iterator.hasNext();) {
	...
	public IConfigurationElement[] getConfigurationElementsFor(
		...
		IExtensionPoint p = (IExtensionPoint)extensionpoints.get(extensionPointId);
		return  (p != null ) ? p.getConfigurationElements() : new IConfigurationElement[0];
	}

	public IConfigurationElement[] getConfigurationElementsFor(
			String namespace, String extensionPointName) {
		String pid = context.getNameStrategy().getUniqueIdentifier(namespace, extensionPointName);
	...
	public IConfigurationElement[] getConfigurationElementsFor(
		...
		return  (ext != null ) ? ext.getConfigurationElements() : new IConfigurationElement[0];
	}
	...
	public IExtensionPoint[] getExtensionPoints(String namespace) {
		...
		for(Iterator itr = extensionpoints.entrySet().iterator();itr.hasNext();){
			Entry entry = (Entry)itr.next();
			String pid = (String)entry.getKey();
			IExtensionPoint p = (IExtensionPoint)entry.getValue();
			if(context.getNameStrategy().getNamespaceFromIdentifier(pid).equals(namespace)){
				list.add(p);
			}
		}
	...
	public IExtension[] getExtensions(String namespace) {
		...
		for(Iterator itr = extensions.entrySet().iterator();itr.hasNext();){
			...
			if(context.getNameStrategy().getNamespaceFromIdentifier(extid).equals(namespace)){
	...
	public String[] getNamespaces() {
		...
		for(Iterator itr = extensionpoints.entrySet().iterator();itr.hasNext();){
			Entry entry = (Entry)itr.next();
			String ns = context.getNameStrategy().getNamespaceFromIdentifier((String)entry.getKey());
			if(!list.contains(ns)){
				list.add(ns);
			}
		}
	...
	public boolean removeExtension(IExtension extension)
		...
		if(ext != null){
			...
			IExtensionPoint extensionPoint = (IExtensionPoint)extensionpoints.get(pid);
			if(extensionPoint != null){
				context.getObjectFactory().removeExtensionFrom(extensionPoint, ext);
			}			
	...
	public boolean removeExtensionPoint(IExtensionPoint extensionPoint) throws IllegalArgumentException {
		String pid = extensionPoint.getUniqueIdentifier();
		extensionPoint = (IExtensionPoint)extensionpoints.remove(pid);
		if(extensionPoint != null){
			IExtension[] exts = extensionPoint.getExtensions();
			List<IExtensionDelta> deltas = new LinkedList<IExtensionDelta>();
			deltas.add(new ExtensionDelta(extensionPoint,IExtensionDelta.REMOVED));
