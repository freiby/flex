...
public class XMLConfigurationElement implements IConfigurationElement,Comparable<XMLConfigurationElement> {
	 ...
	 * 在xml这个树上,如果任何一个子节点上有"filter" 属性,并且它的值和此函数的传入参数匹配，则以此结点为根结点的子树和它的父结点一直上溯到根，这些结点都会被记录在返回的String 里。 
	 * 但并不能保证父结点的其它子结点也包含到此String中，
	 * 
	 * 0 参数为 null 或 "" 或 "*" 不过滤，返回toXML()。
	 * 1 当前结点通过了，把当前结点的子树返回。
	 * 2 如果当前结点没有通过，
	 * 		2.1 当前结点还有子结点，则先过滤子结点，如果过滤后的String 长度不为"0",则返回: 当前结点name + attribute + 过滤结果 + value + 当前结点的xml结束符。
	...
	public String toXMLWithFilter(String filter) {
		...
		if(children.size() != 0){
			...
			//如果过滤后的String 长度不为"0",则返回: 当前结点name + attribute + 过滤结果 + value + 当前结点的xml结束符。
			if(buf.length()>0){
				// 当前结点name + attribute
...
//							'"');
	 ...
	 * 					如果结果不是blank则  返回：当前结点name + attribute + 结果 + value + 当前结点的xml结束符。
	 * 		1.2 如果当前元素匹配成功
	 * 			返回null
	 * 2 如果当前元素没有blacklist属性
	 * 			如果没有子元素，则返回当前元素toString。
	 * 			如果有子元素
	 * 				记算子元素的String形式，
	 * 					如果结果blank，则返回当前元素toString。
	 ...
	 *  
	 *  如果为"*"       则任何有blacklist属性的元素都匹配成功，即使blacklist的值为"" ，除了没有blacklist属性的元素。
	 *  如果为null或""  则认为匹配不成功，此函数返回值与toXML()同。
	 *  
	 *  
	 * @return
	 */
...
//						'"');
	...
	private boolean blackListMatch(String blacklist,String blacklists){
		...
		//如果为"*"       则过滤任何有blacklist属性的元素，即使blacklist的值为""。，除了没有blacklist属性的元素。 能进入此方法说明一定有blacklist属性
		if(blacklist.equals("*")){
			return true;
		}
