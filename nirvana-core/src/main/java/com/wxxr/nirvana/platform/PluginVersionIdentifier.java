...
public class PluginVersionIdentifier implements Comparable<PluginVersionIdentifier>{
  ...
  public PluginVersionIdentifier(int major,int minor, int revision, String buildId){
    this.major = major;
    this.minor = minor;
  ...
  public PluginVersionIdentifier(String version){
    ...
    if((vs.length < 2)||(!StringUtils.isNumeric(vs[0]))||(!StringUtils.isNumeric(vs[1]))||(vs.length > 3)){
  ...
  public String toString(){
    StringBuffer buf = new StringBuffer().append(major).append('.').append(minor).append('.').append(revision);
