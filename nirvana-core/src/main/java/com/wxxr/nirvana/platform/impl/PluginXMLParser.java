...
public class PluginXMLParser implements ElementParserProvider {
    ...
    elementInstances.put(CoreConstants.EXTENSION, ExtensionElementHandler.class);
    elementInstances.put(CoreConstants.EXTENSION_POINT, ExtensionPointElementHandler.class);  
	}
	
	public static void registerElementParser(String elementName, Class parserClass){
	...
	public IConfigureElementHandler getElementParser(String elementTag) {
		...
		try {
		  return (IConfigureElementHandler)cls.newInstance();
			...
			throw new RuntimeException("Failed to create Configure object from class : "+cls,e);
		}
	}
	/**
	...
	public PluginConfigurationElement parse(InputStream in) throws Exception {
			...
			XMLConfigurationElementHandler handler = new XMLConfigurationElementHandler(this);
