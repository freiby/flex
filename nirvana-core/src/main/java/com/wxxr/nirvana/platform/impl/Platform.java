...
//	private UnifiedLoaderRepository3 loaderRepository = new UnifiedLoaderRepository3();
	...
	private INameStrategy nameStrategy = new INameStrategy() {
		...
		public String getUniqueIdentifier(String namespace, String name) {
			...
			return new StringBuffer(namespace).append('.').append(name).toString();
		}
	...
	private IExtensionObjectFactory objectFactory = new IExtensionObjectFactory() {
		...
		/* (non-Javadoc)
		 * @see com.wxxr.web.platform.core.IExtensionObjectFactory#addExtensionTo(com.wxxr.web.platform.core.IExtensionPoint, com.wxxr.web.platform.core.IExtension)
		 */
		...
		/* (non-Javadoc)
		 * @see com.wxxr.web.platform.core.IExtensionObjectFactory#createExecutableExtension(com.wxxr.web.platform.core.IContributor, java.lang.String, java.lang.String)
		 */
		...
		/* (non-Javadoc)
		 * @see com.wxxr.web.platform.core.IExtensionObjectFactory#createExtension(com.wxxr.web.platform.core.IExtensionRegistry, java.lang.Object)
		 */
		public IExtension createExtension(IExtensionRegistry registry, Object source) {
			if(source instanceof ExtensionConfigurationElement){
				Extension ext = new Extension((ExtensionConfigurationElement)source);
				return ext;
			}else{
				throw new IllegalArgumentException("source is not instanceof ExtensionConfigurationElement");
			}
		}

		/* (non-Javadoc)
		 * @see com.wxxr.web.platform.core.IExtensionObjectFactory#createExtensionObject(com.wxxr.web.platform.core.IExtensionRegistry, java.lang.Object)
		 */
		public Object createExtensionObject(IExtensionRegistry registry, Object source) {
		...
		/* (non-Javadoc)
		 * @see com.wxxr.web.platform.core.IExtensionObjectFactory#createExtensionPoint(com.wxxr.web.platform.core.IExtensionRegistry, java.lang.Object)
		 */
		public IExtensionPoint createExtensionPoint(IExtensionRegistry registry,
				Object source) {
			if(source instanceof ExtensionPointConfigurationElement){
				ExtensionPoint ext = new ExtensionPoint((ExtensionPointConfigurationElement)source);
				return ext;
			}else{
				throw new IllegalArgumentException("source is not instanceof ExtensionPointConfigurationElement");
			}
		}

		/* (non-Javadoc)
		 * @see com.wxxr.web.platform.core.IExtensionObjectFactory#createPlugin(com.wxxr.web.platform.core.IExtensionRegistry, java.lang.Object)
		 */
		public IPluginDescriptor createPlugin(IExtensionRegistry registry,
				Object source) {
			if(source instanceof PluginConfigurationElement){
				Plugin plugin = new Plugin((PluginConfigurationElement)source,registry);
				return plugin;
			}else{
				throw new IllegalArgumentException("Source is not instanceof PluginConfigurationElement!");
			}
		}

		/* (non-Javadoc)
		 * @see com.wxxr.web.platform.core.IExtensionObjectFactory#getContributor(java.lang.String)
		 */
		...
		/* (non-Javadoc)
		 * @see com.wxxr.web.platform.core.IExtensionObjectFactory#removeExtensionFrom(com.wxxr.web.platform.core.IExtensionPoint, com.wxxr.web.platform.core.IExtension)
		 */
	...
	private Map<String, Plugin> plugins = new ConcurrentHashMap<String, Plugin>();//actived plugins
	...
	private TimerTask scanTask = new TimerTask() {
		...
		public void run() {
			try {
				...
				log.warn("Caught exception when scan plugin root :"+pluginRootDir, e);
			}
		}
...
//		loaderRepository.registerClassLoader((UnifiedClassLoader3)platformClassLoader);
	...
	/* (non-Javadoc)
	 * @see com.wxxr.web.platform.core.ICoreContext#getContributor(java.lang.String)
	 */
	...
	/* (non-Javadoc)
	 * @see com.wxxr.web.platform.core.IPlatform#getPluginDescriptor(java.lang.String)
	 */
	...
	/* (non-Javadoc)
	 * @see com.wxxr.web.platform.core.IPlatform#deployPluginByXml(java.lang.String)
	 */
...
//		    	throw new Exception("pluginXmlString is not contain plugin id  or version info.");
//		    }
//		    File dir = new File(pluginRootDir + "\\" + pluginId + "\\" +version);
//		    dir.mkdirs();
//		    File f = new File(pluginRootDir + "\\" + pluginId + "\\" +version + "\\webplugin.xml");
//		    if(f.createNewFile()){
...
//			throw new CoreException("Failed to deploy plugin : " + pluginXmlString,e);
//		}
//
//	}
	...
	/**
	 * @param pluginRootDir the pluginRootDir to set
	 */
	...
	/* (non-Javadoc)
	 * deployPlugin step:
	 * 	1 get the plugin's PulginConfigureElement 
	 *  2 mkdir "pluginRootDir/pulginId/pulginVersion"
	 *  3 unjar to the new dir
	 *  4 pluginConfig.setPluginxmlURL(unjar file dir)
	 *  5 regist to Platform.versions
	 *  6 set the plugin's PulginConfigureElement is deployed
	 *  
...
//			if(conn instanceof JarURLConnection){
				...
				try {
					for (JarEntry entry = jin.getNextJarEntry();(pluginConfig == null)&&(entry != null) ; entry = jin.getNextJarEntry())
					{
				...
				if(pluginConfig != null){
					File deployDir = new File(pluginRootDir+"/"+pluginConfig.getNamespaceIdentifier(),pluginConfig.getAttribute(Plugin.PLUGIN_VERSION));
					if (log.isInfoEnabled()){ 
						log.info("Unzip package :"+packageURL+" to deploy directory "+ deployDir.getAbsolutePath());
					}
					deployDir.mkdirs();
					...
					if (log.isInfoEnabled()){ 
						log.info("Package :"+packageURL+" was unzipped to directory " + deployDir.getAbsolutePath());
					}
					pluginConfig.setPluginxmlURL(new File(deployDir,CoreConstants.PLUGIN_XML).toURL());
...
//			}
	...
	protected Plugin activate(PluginConfigurationElement pluginConfig) throws Exception {
		if(pluginConfig == null){
			throw new IllegalArgumentException();
		}
		String deployURL = new File(pluginConfig.getPluginxmlURL().getFile()).getParentFile().toURL().toExternalForm();
		if (log.isInfoEnabled()){ 
			log.info("Deploying plugin "+ pluginConfig.getNamespaceIdentifier());
		}
		Plugin plugin = (Plugin)getObjectFactory().createPlugin(registry, pluginConfig);
		plugin.setInstallURL(deployURL);
		plugin.setMessageResourceBundle(new PluginMessageResourceBundle(deployURL));
		plugin.setPluginClassLoader(createPluginClassLoader(plugin.getRuntimeLibrary()));
		...
		for(int i=0 ; (elems != null)&&(i < elems.length) ; i++){			
			if(log.isDebugEnabled()){
				...
				log.debug("Deploy element id:" + elems[i].getAttribute(CoreConstants.EXTENSION_ID));
			}
			registry.addContribution(elems[i], pluginConfig.getContributor(), elems[i].getName(), plugin.getMessageResourceBundle());
		}
		
		if (log.isInfoEnabled()){ 
			log.info("Plugin: " + pluginConfig.getNamespaceIdentifier() + " was deployed !");
		}
		return plugin;
	}
...
//        for (int i = 0; (elems != null) && (i < elems.length); i++) {
//            String extentionPointString = elems[i].getAttribute(CoreConstants.EXTENSION_TARGET);
//            if(StringUtils.isBlank(extentionPointString)){
//                String extensionId = elems[i].getAttribute(CoreConstants.EXTENSION_ID);
...
//                    map.put("1", elems[i]);
//                } else if (extentionPointString.equalsIgnoreCase("com.wxxr.web.ui.menus")) {
//                    map.put("2", elems[i]);
//                } else if (extentionPointString.equalsIgnoreCase("com.wxxr.web.ui.views")) {
//                    map.put("3", elems[i]);
//                } else if (extentionPointString.equalsIgnoreCase("com.wxxr.web.ui.pages")) {
//                    map.put("4", elems[i]);
//                } else {
//                    String extensionId = elems[i].getAttribute(CoreConstants.EXTENSION_ID);
//                    if(StringUtils.isBlank(extensionId)){
...
//        }
	...
	protected void deactivate(Plugin plugin) {
		...
		if(plugin.isPluginInitialized() &&(plugin.getPluginInitializer() != null)){
			try {
				plugin.getPluginInitializer().destroy(plugin);
			} catch (Exception e) {
				log.warn("Failed to deactivate plugin :"+plugin.getUniqueIdentifier(), e);
			}
		}
		...
		PluginClassLoader cl = pluginClassLoaders.remove(plugin.getUniqueIdentifier());
		if(cl != null){
			cl.destroy();
		}
...
//		EventManager.getInstance().notifyEvent(event);
	...
	/* (non-Javadoc)
	 * @see com.wxxr.web.platform.core.IPlatform#getLatestDeployedPluginVersion(java.lang.String)
	 */
	...
	protected PluginVersionIdentifier getLatestDeployedVersion(String pluginId) {
		...
		Map<PluginVersionIdentifier,PluginConfigurationElement> versions = deployedPluginVersions.get(pluginId);
		...
		TreeSet<PluginVersionIdentifier> set = new TreeSet<PluginVersionIdentifier>(versions.keySet());
		return set.last();   
	}

	/* (non-Javadoc)
	 * @see com.wxxr.web.platform.core.IPlatform#isPluginDeployed(java.lang.String, java.lang.String)
	 */
	...
	protected PluginConfigurationElement getPluginVersion(String pluginId, String version){
		return getPluginVersion(pluginId,new PluginVersionIdentifier(version));    
	}

	protected PluginConfigurationElement getPluginVersion(String pluginId, PluginVersionIdentifier version){
	 ...
	 *  2 remove  pulgin  from deployedPluginVersions ,and  change it to deActive state at the same time
	 *  
	 * @see com.wxxr.web.platform.core.IPlatform#undeployPlugin(java.lang.String, java.lang.String)
	 */
	...
	throws CoreException {
		synchronized (c) {
			...
			if (version == null) {
				...
				try {
					...
					if (deployDir.exists()) {
						...
						File deployRoot = new File(pluginRootDir + "/" + pluginId);
						if (deployRoot.exists()) {//del the version when after del this version the plugin have none version in deployDir 
					...
					if(configs.size()<1){//del the version when after del this version the plugin have none version in deployDir 
						deployedPluginVersions.remove(pluginId);
					}
					// 3 change it to deActive state at the same time
					deactivate((Plugin) getPluginDescriptor(pluginId));
					log.info(" plugin :" + pluginId + " of version :" + version+"was undeploy !");
				} catch (IOException ee) {
					log.warn(" can't delete pluginRootDir" + "/" + pluginId + "/"
							+ version);
			...
			String[] deployedPluginIds = deployedPluginVersions.keySet().toArray(new String[0]);
			if ((deployedPluginIds != null) && (deployedPluginIds.length > 0)) {
				for (int i = 0; i < deployedPluginIds.length; i++) {
					...
					String version2 = currentActivedPluginVersion.get(pluginId2);//the active version
					if(version2 == null){
					    log.error(pluginId2 + "is include of deployedPluginVersions but not in currentActivedPluginVersion !! " );
					    continue;
					}
					PluginConfigurationElement pluginConfigurationElement = (PluginConfigurationElement) deployedPluginVersions.get(pluginId2).get(new PluginVersionIdentifier(version2));
					// maybe throw exception because version is null
					IConfigurationElement[] dependElem = pluginConfigurationElement.getChildren(CoreConstants.PLUGIN_DEPEND);
					if (dependElem != null && dependElem.length > 0) {
						int index = dependElem[0].getValue().indexOf(pluginId);
						if(index != -1){
						    if(log.isDebugEnabled()){
						        log.debug("when undeploy "+pluginId+" "+version +" find "+ pluginId2+" "+version2+"  depend it  so undeploy them at the same time .");
						    }
	...
	/* (non-Javadoc)
	 * @see com.wxxr.web.platform.core.IPlatform#createPluginClassLoader(com.wxxr.web.platform.core.IRuntimeLibrary)
	 */
	protected ClassLoader createPluginClassLoader(IRuntimeLibrary runtimeLibray) {
//		if((runtimeLibray == null)||(runtimeLibray.getJarFiles() == null)||(runtimeLibray.getJarFiles().length == 0)){
//			return platformClassLoader;
//		}
//		PluginClassLoader loader = new PluginClassLoader(runtimeLibray,parentClassLoader,loaderRepository);
...
//		return loader;
	...
	/* (non-Javadoc)
	 * @see com.wxxr.web.platform.core.IPlatform#activatePlugin(java.lang.String)
	 */
	public void activatePlugin(String pluginId, String version) throws CoreException {
		if(!isPluginActivated(pluginId, version)){
			if(!isPluginDeployed(pluginId, version)){
				throw new CoreException("Plugin :"+pluginId+" of version :"+version+" is not deployed !");
			}
...
//				EventManager.getInstance().notifyEvent(event);
	...
	/* (non-Javadoc)
	 * @see com.wxxr.web.platform.core.IPlatform#deActivatePlugin(java.lang.String, java.lang.String)
	 */
	...
	protected void scanPluginRoot() throws Exception{
		synchronized (c) {
			...
			for (int i = 0; i < plugins.length; i++) {
				...
				if(dir != null){
					if(log.isDebugEnabled()){
						log.debug("Scan plugin versions under folder :" + dir.getAbsolutePath());
					}
	...
	private PluginConfigurationElement initPluginRoot() throws Exception {
		...
		if(!pluginRoot.exists()){
			if(!pluginRoot.mkdirs()){
				throw new CoreException("Failed to create plugin root directory :"+pluginRootDir);        
			}
		}
		...
		if(corePluginXML != null){
			...
			if(!xmlFile.exists()){
				if(log.isInfoEnabled()){
					log.info("Copy platform core plugin xml to plugin directory:"+corePluginDir.getAbsolutePath());
				}
				FileUtils.copyURLToFile(corePluginXML, xmlFile);
			}else{
				if(corePluginXML.openConnection().getLastModified() > xmlFile.lastModified()){
					FileUtils.copyURLToFile(corePluginXML, xmlFile);
				}
			}
			return pluginConfig;
	...
	protected void scanPluginVersions(File pluginDir){
		...
		Map<PluginVersionIdentifier,PluginConfigurationElement> configs = deployedPluginVersions.get(pluginName);
		if(configs == null){
		...
		for (int i = 0; i < versions.length; i++) {
			...
			if(configs.containsKey(new PluginVersionIdentifier(pathname.getName()))){
			...
			try {
				...
				if(pluginxml.exists()){
					if(log.isInfoEnabled()){
						log.info("Found plugin xml under folder :"+pathname.getAbsolutePath());
					}
					URL xmlURL = pluginxml.toURL();
					InputStream ins = xmlURL.openStream();
					try {
						PluginConfigurationElement pluginConfig = parser.parse(ins);
						if(log.isInfoEnabled()){
							log.info("Plugin xml was parsed successfully!" + pluginxml.getAbsolutePath());
						}
						pluginConfig.setPluginxmlURL(xmlURL);
						configs.put(pluginConfig.getPluginVersion(),pluginConfig);
					} finally {
						try {
							ins.close();
						} catch (Exception e) {
				...
				log.warn("Failed to scan plugin version under folder :"+pathname.getAbsolutePath(),e);
			}
		}
	}
	...
	public void activatePlugins() throws Exception {
		...
		if((deployedPluginIds != null)&&(deployedPluginIds.length > 0)){
			for (int i = 0; i < deployedPluginIds.length; i++) {
				...
				IConfigurationElement[] dependElem = pluginConfigurationElement.getChildren(CoreConstants.PLUGIN_DEPEND);
				if (dependElem != null && dependElem.length > 0) {
					if(dependElem[0] != null && dependElem[0].getValue() != null ){
	...
	private LinkedList<PluginVersion> resolveDependencyOrder(LinkedList<PluginVersion> startingPlugins){
//		return startingPlugins;
		...
		for (Iterator iterator = startingPlugins.iterator(); iterator.hasNext();) {
			...
			for (Iterator iterator2 = dependSet.iterator(); iterator2.hasNext();) {
				...
				PluginVersion dependPlug = getPluginVersionFromStartingPlugins(dependPlugId,startingPlugins);
				if(dependPlug == null){ 
					log.warn(pluginVersion.pluginId+" is depend "+dependPlugId+", the "+dependPlugId+"is not deployed!  so "+pluginVersion.pluginId+" can't be actived!" );
					insertIndex = -1;
					break;
				}
	...
	private PluginVersion getPluginVersionFromStartingPlugins(String pluginId,LinkedList<PluginVersion> startingPlugins){
		PluginVersion returnPluginVersion = null;
		for (Iterator iterator = startingPlugins.iterator(); iterator.hasNext();) {
			PluginVersion pluginVersion = (PluginVersion) iterator.next();
			if(pluginId.equals(pluginVersion.pluginId)){
				returnPluginVersion = pluginVersion; // if one pluginid has two version in LinkedList return anyone
			}
		}
	...
	/* (non-Javadoc)
	 * @see com.wxxr.web.platform.core.IPlatform#isPluginActivated(java.lang.String, java.lang.String)
	 */
	public boolean isPluginActivated(String pluginId, String version) {
		IPluginDescriptor plugin = getPluginDescriptor(pluginId);
		if(version != null){
			return (plugin != null)&&(plugin.getVersionIdentifier().equals(new PluginVersionIdentifier(version)));
		}else{
			return (plugin != null);
		}
	}
	...
	public Object createPluginObject(String pluginId, String className,
		...
		return createPluginObject(getPluginDescriptor(pluginId),className,parameters);
	}
	...
	public String[] listAllPluginNames() {
		synchronized(deployedPluginVersions){
			...
			return deployedPluginVersions.keySet().toArray(new String[deployedPluginVersions.size()]);
		}
	}
	...
	public String[] listAllVersionsOfPlugin(String pluginId){
		synchronized (deployedPluginVersions) {
			...
			for (Iterator itr = versions.keySet().iterator(); itr.hasNext();) {
				PluginVersionIdentifier v = (PluginVersionIdentifier) itr.next();
				if(v != null){
	...
	public void registerWebClassLoader(URLClassLoader classLoader) {
		...
		((CompositeClassLoader)parentClassLoader).setWebClassLoder(classLoader);
	}
