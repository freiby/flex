...
public abstract class BaseExtensionPointManager implements IRegistryChangeListener {
	...
	/* (non-Javadoc)
	 * @see com.wxxr.web.platform.core.IRegistryChangeListener#registryChanged(com.wxxr.web.platform.core.IRegistryChangeEvent)
	 */
	public void registryChanged(IRegistryChangeEvent event) {
		if(event == null){
			return;
		}
		IExtensionDelta[] deltas = event.getExtensionDeltas(namespace,extensionPointId);
		IExtension ext = null;
	...
	public void start() {
		...
		IExtensionPoint point = platform.getExtensionRegistry().getExtensionPoint(namespace,extensionPointId);
		...
		getUIPlatform().getExtensionRegistry().addRegistryChangeListener(this, namespace);
	}
	
	public void stop(){
		getUIPlatform().getExtensionRegistry().removeRegistryChangeListener(this);
	}
	...
	throws Exception {
		return PlatformLocator.getPlatform().createPluginObject(plugin, clazz, null);
	}
