...
public class SMDActionTest1 extends ActionSupport {
	...
	public void doSomethingPrimitives(String stringParam, int intParam,
			boolean booleanParam, char charParam, long longParam,
			float floatParam, double doubleParam, short shortParam,
			byte byteParam) {
	...
	@SMDMethod
	public List<City> getList(String name) {
		List<City> citys = new ArrayList<City>();
		City c = null;
		if("zhangsan".equals(name)){
			c = new City("Tanmay","Bangalore","560001");
			citys.add(c);
		}else if("wangwu".equals(name)){
			c = new City("Sachin","Mumbai","400003");
			citys.add(c);
		}else {
