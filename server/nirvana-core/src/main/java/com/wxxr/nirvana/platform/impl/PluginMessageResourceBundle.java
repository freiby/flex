 ...
 *
 */
public class PluginMessageResourceBundle implements MessageResourceBundle {

	private final static Log log = LogFactory
			.getLog(PluginMessageResourceBundle.class);
	 ...
	 * 
	 * @param installURL
	 *            is the path that webplugin.xml in it .
	 */
	 ...
	 * 
	 * @see
	 * com.wxxr.web.platform.core.MessageResourceBundle#getLocalizedMessage(
	 * java.lang.String)
	 */
	public String getLocalizedMessage(String key) {
		...
		if ((bundle = getResourceBundle()) != null) {
			try {
				...
				return new String(returnS.getBytes("ISO-8859-1"), "utf8")
						.intern();
			} catch (UnsupportedEncodingException e) {
				if (log.isInfoEnabled()) {
					log.info(
							"Failed to convert message from ISO-8859-1 to UTF8",
							e);
				}
	...
	public String getLocalizedMessage(String key, Locale locale) {
		if (getResourceBundle() != null) {
			// code change ISO-8859-1 is java load properties default codestyle
			// utf8 is the propertiesfile's codestyle generally the style is os
			// default codestyle ( the program run on)
	 ...
	 * 
	 * @see
	 * com.wxxr.web.platform.core.MessageResourceBundle#getResourceBundle(java
	 * .util.Locale)
	 */
	...
	private ResourceBundle getBundle(Locale locale) {
		...
		try {
			if (bundleClassLoader == null) {
				URL url = new File(pluginInstallURL, propertiesPath).toURI()
						.toURL();
				URL[] urls = new URL[] { url };
				bundleClassLoader = new URLClassLoader(urls);
			}
			resourceBundle = PropertyResourceBundle.getBundle(propertiesName,
					locale, bundleClassLoader);

		} catch (Exception e) {
			log.info("Failed to load message bundle :"
