 ...
 *
 */
public class ThemeManager extends BaseExtensionPointManager implements
		IThemeManager {
	...
	protected void processExtensionAdded(IExtension ext) {
		...
		for (int i = 0; i < configs.length; i++) {
			...
			if ((elem != null)
					&& THEME_ELEMENT_NAME.equalsIgnoreCase(elem.getName())) {
				try {
					...
					log.warn(
							"Failed to create theme from configuration element :"
									+ elem, e);
				}
			}
		}
